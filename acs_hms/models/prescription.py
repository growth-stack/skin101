...
class ACSPrescriptionOrder(models.Model):
    ...
    def _current_user_doctor(self):
        physician_id = False
        ids = self.env['hms.physician'].search(
    ...
    READONLY_STATES = {'cancel': [('readonly', True)], 'prescription': [
        ('readonly', True)]}

    name = fields.Char(size=256, string='Prescription ID',
                       help='Type in the ID of this prescription', readonly=True)
    diseases_id = fields.Many2one(
        'hms.diseases', ondelete="set null", string='Disease', states=READONLY_STATES)
    group_id = fields.Many2one('medicament.group', ondelete="set null",
                               string='Medicaments Group', states=READONLY_STATES)
    patient_id = fields.Many2one('hms.patient', ondelete="restrict", string='Patient',
                                 required=True, states=READONLY_STATES, tracking=True)
    pregnancy_warning = fields.Boolean(
        string='Pregnancy Warning', states=READONLY_STATES)
    notes = fields.Text(string='Prescription Notes', states=READONLY_STATES)
    prescription_line_ids = fields.One2many(
        'prescription.line', 'prescription_id', string='Prescription line', states=READONLY_STATES)
    pharmacy = fields.Many2one(
        'res.company', ondelete="cascade", string='Pharmacy', states=READONLY_STATES)
    ...
    patient_age = fields.Char(related='patient_id.age',
                              string='Age', store=True, readonly=True)
    treatment_id = fields.Many2one(
        'hms.treatment', 'Treatment', states=READONLY_STATES)
    ...
    old_prescription_id = fields.Many2one(
        'prescription.order', 'Old Prescription')
    ...
    def button_confirm(self):
        for app in self:
            if not app.prescription_line_ids:
                raise UserError(
                    _('You cannot confirm a prescription order without any order line.'))
    ...
    def onchange_patient(self):
        if self.patient_id:
            prescription = self.search([('patient_id', '=', self.patient_id.id), (
                'state', '=', 'prescription')], order='id desc', limit=1)
    ...
    def action_prescription_send(self):
        ...
        try:
            template_id = ir_model_data.get_object_reference(
                'acs_hms', 'acs_prescription_email')[1]
        except ValueError:
            template_id = False
        try:
...
class ACSPrescriptionLine(models.Model):
    ...
    prescription_id = fields.Many2one(
        'prescription.order', ondelete="cascade", string='Prescription')
    product_id = fields.Many2one('product.product', ondelete="cascade", string='Product', required=True, domain=[
                                 ('hospital_product_type', '=', 'medicament')])
    allow_substitution = fields.Boolean(string='Allow Substitution')
    prnt = fields.Boolean(
        string='Print', help='Check this box to print this line of the prescription.', default=True)
    quantity = fields.Float(
        string='Units',  help="Number of units of the medicament. Example : 30 capsules of amoxicillin", default=1.0)
    active_component_ids = fields.Many2many(
        'active.comp', 'product_pres_comp_rel', 'product_id', 'pres_id', 'Active Component')
    dose = fields.Float(
        'Dosage', help="Amount of medication (eg, 250 mg) per dose", default=1.0)
    form_id = fields.Many2one('drug.form', related='product_id.form_id',
    ...
    common_dosage_id = fields.Many2one('medicament.dosage', ondelete="cascade",
                                       string='Dosage/Frequency', help='Drug form, such as tablet or gel')
    short_comment = fields.Char(
        string='Comment', help='Short comment on the specific drug')
    appointment_id = fields.Many2one(
        'hms.appointment', ondelete="restrict", string='Appointment')
    ...
    def onchange_product(self):
        if self.product_id:
            self.active_component_ids = [
                (6, 0, [x.id for x in self.product_id.active_component_ids])]
