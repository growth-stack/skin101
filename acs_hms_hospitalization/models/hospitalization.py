...
class Hospitalization(models.Model):
    ...
    def _compute_checklist_done(self):
        for rec in self:
            if rec.checklist_ids:
                done_checklist = rec.checklist_ids.filtered(
                    lambda s: s.is_done)
                rec.checklist_done = (len(done_checklist)
                                      * 100)/len(rec.checklist_ids)
            else:
    ...
    def _compute_pre_ward_checklist_done(self):
        for rec in self:
            if rec.pre_ward_checklist_ids:
                done_checklist = rec.pre_ward_checklist_ids.filtered(
                    lambda s: s.is_done)
                rec.pre_ward_checklist_done = (
                    len(done_checklist) * 100)/len(rec.pre_ward_checklist_ids)
            else:
    ...
    def _rec_count(self):
        ...
        for rec in self:
            rec.invoice_count = Invoice.sudo().search_count(
    ...
    name = fields.Char(string='Hospitalization#',
                       copy=False, default="Hospitalization#")
    ...
    patient_id = fields.Many2one('hms.patient', ondelete="restrict",
                                 string='Patient', states=READONLY_STATES, tracking=True)
    ...
    relative_number = fields.Char(
        string='Patient Relative Number', states=READONLY_STATES)
    ward_id = fields.Many2one(
        'hospital.ward', ondelete="restrict", string='Ward/Room', states=READONLY_STATES)
    bed_id = fields.Many2one(
        'hospital.bed', ondelete="restrict", string='Bed No.', states=READONLY_STATES)
    ...
    discharge_date = fields.Datetime(
        string='Discharge date', states=READONLY_STATES)
    no_invoice = fields.Boolean(
        string='Invoice Exempt', states=READONLY_STATES)
    ...
    accommodation_count = fields.Integer(
        compute='_rec_count', string='# Accommodation History')
    physician_id = fields.Many2one(
        'hms.physician', string='Primary Physician', states=READONLY_STATES, tracking=True)
    ...
    checklist_ids = fields.One2many('inpatient.checklist', 'hospitalization_id',
                                    ...
                                    states=READONLY_STATES)
    checklist_done = fields.Float(
        'Admission Checklist Done', compute='_compute_checklist_done', store=True)
    pre_ward_checklist_ids = fields.One2many('pre.ward.check.list', 'hospitalization_id',
                                             ...
                                             states=READONLY_STATES)
    pre_ward_checklist_done = fields.Float(
        'Pre-Ward Checklist Done', compute='_compute_pre_ward_checklist_done', store=True)
    ...
    clinincal_history = fields.Text(
        string="Clinical Summary", states=READONLY_STATES)
    ...
    adv_on_dis = fields.Text(
        string="Advice on Discharge", states=READONLY_STATES)

    discharge_diagnosis = fields.Text(
        string="Discharge Diagnosis", states=READONLY_STATES)
    op_note = fields.Text(string="Operative Note", states=READONLY_STATES)
    post_operative = fields.Text(
        string="Post Operative Course", states=READONLY_STATES)
    ...
    fir_reason = fields.Char(
        string="If not reported to police give reason", states=READONLY_STATES)
    ...
    ward_rounds = fields.One2many(
        'ward.rounds', 'hospitalization_id', string='Ward Rounds', states=READONLY_STATES)
    discharge_plan = fields.Text(
        string='Discharge Plan', states=READONLY_STATES)
    move_ids = fields.One2many(
        'stock.move', 'hospitalization_id', string='Moves', states=READONLY_STATES)
    invoice_count = fields.Integer(compute='_rec_count', string='# Invoices')
    prescription_ids = fields.One2many(
        'prescription.order', 'hospitalization_id', 'Prescriptions')
    prescription_count = fields.Integer(
        compute='_rec_count', string='# Prescriptions')
    surgery_ids = fields.One2many(
        'hms.surgery', 'hospitalization_id', "Surgeries")
    ...
    death_register_id = fields.Many2one(
        'patient.death.register', string='Death Register', states=READONLY_STATES)

    _sql_constraints = [('name_uniq', 'UNIQUE(name)',
                         'Hospitalization must be unique!')]

    @api.model
    def create(self, values):
        patient_id = values.get('patient_id')
        active_hospitalizations = self.search(
            [('patient_id', '=', patient_id), ('state', 'not in', ['cancel', 'done', 'discharged'])])
        if active_hospitalizations:
            raise ValidationError(
                _("Patient Hospitalization is already active at the moment. Please complete it before creating new."))
        if values.get('name', 'Hospitalization#') == 'Hospitalization#':
            values['name'] = self.env['ir.sequence'].next_by_code(
                'acs.hospitalization') or 'Hospitalization#'
    ...
    def action_prescription(self):
        action = self.env.ref(
            'acs_hms.act_open_hms_prescription_order_view').read()[0]
    ...
    def action_accommodation_history(self):
        action = self.env.ref(
            'acs_hms_hospitalization.action_accommodation_history').read()[0]
        action['domain'] = [('hospitalization_id', '=', self.id)]
        action['context'] = {
            'default_patient_id': self.patient_id.id,
            'default_hospitalization_id': self.id}
    ...
    def action_view_surgery(self):
        action = self.env.ref(
            'acs_hms_surgery.act_open_action_form_surgery').read()[0]
        action['domain'] = [('hospitalization_id', '=', self.id)]
        action['context'] = {
            'default_patient_id': self.patient_id.id,
            'default_hospitalization_id': self.id}
    ...
    def consume_hopitalization_material(self):
        for rec in self:
            if not rec.company_id.hospitalization_usage_location:
                raise UserError(
                    _('Please define a location where the consumables will be used during the surgery in company.'))
            if not rec.company_id.hospitalization_stock_location:
                raise UserError(
                    _('Please define a hospitalization location from where the consumables will be taken.'))
    ...
    def action_create_invoice(self):
        ...
        invoice_id = self.acs_create_invoice(
            patient=self.patient_id, product_data=product_data, inv_data=inv_data)
        invoice_id.write({
